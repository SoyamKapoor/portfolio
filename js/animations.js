...
function animCount(el, target) {
  ...
  const hasPls = target > 1 && el.dataset.target && el.textContent.includes('+') || el.innerHTML.includes('+');
  const originalText = document.querySelector(`[data-target="${target}"]`)?.textContent || '';
  const hasPlus = originalText.includes('+');
  let s = 0;
  const iv = setInterval(() => {
    s = Math.min(s + 1, target);
    el.textContent = s + (hasPlus && s >= target ? '+' : '');
    if (s >= target) clearInterval(iv);
  }, 1600 / target);
}
    ...
    if (x.isIntersecting) {
      x.target.querySelectorAll('[data-target]').forEach(el => {
        ...
        if (!isNaN(v)) {
          ...
          const iv = setInterval(() => {
            s = Math.min(s + 1, v);
            el.textContent = s + (hasPlus && s >= v ? '+' : '');
            if (s >= v) clearInterval(iv);
          }, 1600 / v);
        }
      });
      counterObserver.unobserve(x.target);
...
document.querySelectorAll('.atiles').forEach(el => counterObserver.observe(el));
